	...
	public static void main(String[] args) {
		...
		for(int i = 0;i<list.size()-1;i++){//控制轮数的
			...
			for(int j = i+1;j<list.size() ;j++){
				...
				List<String> listj = map.get(uidj);//直接拿到的是map中的list，对这个list进行操作会直接影响map的数据
				//retainAll()   取交集的
				//ArrayList<String> newListJ = new ArrayList<>(listj);  //新复制一份
				listj.retainAll(listi);  
				if(listj!=null&&listj.size()>0){
					System.out.println(uidi+"和"+uidj+"共同好友是："+listj);
	...
	private static Map<String,List<String>> getMap() {
		...
		try (BufferedReader br = new BufferedReader(new FileReader("D:\\data\\好友.txt"));){
			...
			while((line = br.readLine())!=null){
				...
				//数组转换为list
				map.put(uid, new ArrayList<String>(Arrays.asList(fss))); //Arrays.asList生成的list不可增删
			}
